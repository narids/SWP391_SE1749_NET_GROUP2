...
//        processRequest(request, response);
        ...
        if (account == null) {
            ...
            if (account.getRole().getRoleId() == 4) {
                ...
                String quizzesByStudentIDSQL = "q.QuizID, q.QuizName, q.QuizContent, q.CreatedDate, q.QuizStatus,\n"
                        + "       cs.ClassID, cs.SubjectID, cs.TeacherID,\n"
                        + "       c.ClassName, s.SubjectName\n"
                        + "FROM ClassSubject cs\n"
                        + "JOIN Quiz q ON cs.QuizID = q.QuizID\n"
                        + "JOIN Class c ON cs.ClassID = c.ClassID\n"
                        + "JOIN Subject s ON cs.SubjectID = s.SubjectID\n"
                        + "JOIN ClassStudent cst ON cs.ClassID = cst.ClassID\n"
                        + "WHERE cst.StudentID =" + String.valueOf(accDao.getStudentByUserID(account.getUserId()));
                ...
                RequestDispatcher dispatcher = request.getRequestDispatcher("jsp/StudentProfileDetail.jsp");
                dispatcher.forward(request, response);
            }
